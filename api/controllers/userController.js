...
const login = async (req, res) => {
    try {
        /* finding One user instead of arrays we can do */
        ...
        if (isUser) {
            ...
            if (comparePassword) {
                ...
                /*1) In destructure iterator cannot work */
                // const { password, isadmin, ...userDetail } = isUser[0]._doc;
                // const { password, isadmin, ...userDetail } = isUser[0]._doc;
             

                /*2) create an object and copy the data exepct password   */
                const userDetail = {};
                Object.keys(isUser[0]._doc).filter((key) => {
                    if (key !== 'password') {
                        userDetail[key] = isUser[0][key];
                    }
                });
...
const updateUser = async (req, res) => {
    try {
        /*1)  finding the user data without password include and update details */
